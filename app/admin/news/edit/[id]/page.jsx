...
export default function EditNewsPage() {
  ...
  const [form, setForm] = useState({
    title: '', content: '', excerpt: '', featuredImage: '', featuredImageType: 'url',
    category: '', status: 'draft',
    isTrending: false, isFeatured: false, tags: '', newsHighlight: '',
  });
    ...
    Promise.all([
      ...
      if (newsData.news) {
        ...
        setForm({
          ...
          category: n.category?._id || n.category || '',
          status: n.status,
          isTrending: n.isTrending || false,
          isFeatured: n.isFeatured || false,
          tags: (n.tags || []).join(', '),
          newsHighlight: n.newsHighlight || '',
        });
      }
      setFetching(false);
    });
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (!canPublish) delete payload.status;
      if (!canTrend) delete payload.isTrending;
      if (!canFeature) delete payload.isFeatured;
        ...
        <div className="bg-white rounded-xl shadow-sm p-6 space-y-4">
          ...
          <div>
            ...
            <input value={form.title} onChange={e => setForm({ ...form, title: e.target.value })}
              className="w-full px-4 py-2.5 border border-slate-200 rounded-lg text-slate-800 focus:outline-none focus:ring-2 focus:ring-red-500" required />
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">Excerpt</label>
            <textarea value={form.excerpt} onChange={e => setForm({ ...form, excerpt: e.target.value })}
              rows={2} className="w-full px-4 py-2.5 border border-slate-200 rounded-lg text-slate-800 focus:outline-none focus:ring-2 focus:ring-red-500 resize-none" />
          </div>
          <div>
            <RichTextEditor
          ...
          <div>
            ...
            <textarea value={form.content} onChange={e => setForm({ ...form, content: e.target.value })}
              rows={12} className="w-full px-4 py-2.5 border border-slate-200 rounded-lg text-slate-800 focus:outline-none focus:ring-2 focus:ring-red-500 resize-y font-mono text-sm" required />
          </div>
          <div>
            ...
            {form.featuredImageType === 'url' ? (
              <input value={form.featuredImage} onChange={e => setForm({ ...form, featuredImage: e.target.value })}
                className="w-full px-4 py-2.5 border border-slate-200 rounded-lg text-slate-800 focus:outline-none focus:ring-2 focus:ring-red-500" placeholder="https://..." />
            ) : (
              <input type="file" accept="image/*" onChange={handleImageUpload}
                className="w-full text-sm text-slate-700" />
            )}
            {form.featuredImage && <img src={form.featuredImage} alt="preview" className="mt-2 h-auto max-h-32 w-full rounded-lg object-contain" />}
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input value={form.tags} onChange={e => setForm({ ...form, tags: e.target.value })}
                className="w-full px-4 py-2.5 border border-slate-200 rounded-lg text-slate-800 focus:outline-none focus:ring-2 focus:ring-red-500" placeholder="politics, economy" />
            </div>
          </div>
